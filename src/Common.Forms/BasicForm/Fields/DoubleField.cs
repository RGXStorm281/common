...
internal class DoubleField(
	...
	double? initialValue
) : FormFieldBase<DoubleField, double?>(id, parent, label, visibilityCondition, validators, initialValue)
{
	...
	public override IFieldDataContainer<double?> GetData() => new BasicFieldDataContainer<double?> { Value = Value };
}
