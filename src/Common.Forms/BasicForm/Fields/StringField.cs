...
internal class StringField(
	...
	string? initialValue
) : FormFieldBase<StringField, string?>(id, parent, label, visibilityCondition, validators, initialValue)
{
	...
	public override IFieldDataContainer<string?> GetData() => new BasicFieldDataContainer<string?> { Value = Value };
}
