...
The cache is implemented as a hosted service, such that it can run in the background of any application.
...
The `TryCache` functions requires an inactivity time span, that allows the cache to delete the item when it hasn't been accessed for long enough. Optionally an additional maximum lifetime (not reset on access) can be set when a model is cached.

```C#
_cache.TryCache(modelKey, model, TimeSpan.FromMinutes(5), TimeSpan.FromMinutes(60));
```

This cache is meant to speed up loading and temporarily store intermediate states, but you should never rely on it for permanent storage, because cached items will be purged eventually.
