...
This library is all about representing complex forms with as simple of an api as possible. While there are a lot of form frameworks around, I found them to be lacking in representing inter-field validation rules and hierarchical model structures. So here we are - hopefully my own solution will handle my absurd requirements ; )
...
-   Modeling form fields, with all data associated:
    -   Its definition, which consists of the data type, its placement in the structure (not necessarily indicating a UI placement!) and rules for validation or loading data from a model and saving changes back to it.
    -   Its state in a running instance, e.g. visibility and validation state or whether user interaction has taken place.
-   Nodes being aware of their context, having clearly defined rules which neighbors are accessible from the current node and how neighboring states affect it.
    -   This allows for cross-field validation (e.g. "this text is required, if that checkbox is checked").
    -   This allows for dynamic visibility of form nodes (e.g. "show this subsection, if that checkbox is checked").
-   Have you noticed the terminology changing from fields to nodes? That is because THE main requirement is to play well with layered, recursive structures, because that is what object-oriented data-structures look like:
    ...
    -   It needs to handle recursive structures (that requirement is very specific for what I want to use this for...).
-   It should offer a (mostly) readable way of building and configuring a form structure, by
    -   using a flow-builder-api with sensible naming, so available options can be explored through autocompletion
    -   assisting with type safety wherever possible
-   It should be extensible wherever possible, to accommodate for edge cases
-   And it should support async operations, in case they are needed by your extensions.

Phew, you might have an idea now why this is not trivial. Next I'll give you some hints on how you might start looking into this project:

## Concept and structure

I highly recommend having a look at the concept diagrams in "/doc/forms-concept" first.

The highest level view on this library is found in "forms_core_components.drawio". There you can see, that the library evolves around the central tree structure in the the namespace "Nodes". It holds the state of the form, can be queried by expressions and search operations, is validated by validator components and can be extended by hooking into lifecycle or pipeline events. And all is managed by the builder components.
