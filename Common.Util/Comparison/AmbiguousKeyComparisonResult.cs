...
/// Matching elements can have a M:N relationship. Difference sets are grouped by key.
/// </summary>
...
public class AmbiguousKeyComparisonResult<TLeft, TRight, TKey>(
	Dictionary<TKey, List<TLeft>> leftDifference, 
	List<(List<TLeft> Left, List<TRight> Right)> intersection, 
	Dictionary<TKey, List<TRight>> rightDifference)
	where TKey : notnull
{
	/// <summary>All elements that occur in the left but not in the right set.</summary>
	public Dictionary<TKey, List<TLeft>> LeftDifference { get; } = leftDifference;

	/// <summary>The tuples of left and right elements that are part of the intersection according to the equality condition.</summary>
	public List<(List<TLeft> Left, List<TRight> Right)> Intersection { get; } = intersection;

	/// <summary>All elements that occur in the right set but not in the left set.</summary>
	public Dictionary<TKey, List<TRight>> RightDifference { get; } = rightDifference;
		...
		Action<TKey, List<TLeft>> createMissing)
	{
		...
		{
			removeObsolete(obsolete.Key, obsolete.Value);
		}
		...
		{
			createMissing(missing.Key, missing.Value);
		}
	}
		...
		TState state)
	{
		...
		foreach (var obsolete in RightDifference)
		{
			removeObsolete(obsolete.Key, obsolete.Value, state);
		}
		...
		foreach (var missing in LeftDifference)
		{
			createMissing(missing.Key, missing.Value, state);
		}
	}
		...
		Func<TKey, List<TLeft>, Task> createMissingAsync)
	{
		...
		foreach (var obsolete in RightDifference)
		{
			await removeObsoleteAsync(obsolete.Key, obsolete.Value);
		}
		...
		foreach (var missing in LeftDifference)
		{
			await createMissingAsync(missing.Key, missing.Value);
		}
	}
		...
		TState state)
	{
		...
		foreach (var obsolete in RightDifference)
		{
			await removeObsoleteAsync(obsolete.Key, obsolete.Value, state);
		}
		...
		foreach (var missing in LeftDifference)
		{
			await createMissingAsync(missing.Key, missing.Value, state);
		}
	}
}